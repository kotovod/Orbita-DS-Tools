...
figma.ui.onmessage = async function(msg) {
  try {
    if (msg.type === 'check-icons') {
      ...
      // Design System Validator - сохранение токенов в памяти
      try {
        ...
        console.log('DSV: Токены успешно сохранены');
      } catch (error) {
        console.error('DSV: Ошибка при сохранении токенов:', error);
      }
      ...
      if (Array.isArray(propertyValue)) {
        ...
        if (hasVisibleValues && shouldReportMissingVariable(node, prop.key)) {
          ...
          nodeIssues.push({
            ...
            nodeId: node.id,
            nodeName: String(node.name || 'Unnamed'),
            value: getPropertyValue(node, prop.key)
          });
        }
  ...
  if (typeof value === 'object' && value.r !== undefined) {
    for (const [id, token] of variables.entries()) {
      ...
      if (tokenValue && typeof tokenValue === 'object' && tokenValue.r !== undefined) {
        // Сравниваем RGB с точностью
        const rMatch = Math.abs(tokenValue.r - value.r) < 0.01;
        const gMatch = Math.abs(tokenValue.g - value.g) < 0.01;
        const bMatch = Math.abs(tokenValue.b - value.b) < 0.01;
        const aMatch = Math.abs((tokenValue.a || 1) - (value.a || 1)) < 0.01;
