 ...
 * @param {Object} node - Нода для обхода
 * @param {Function} callback - Функция обратного вызова для каждой ноды
 * @param {boolean} skipSelf - Пропустить саму ноду (но проверить детей)
 * @param {Object} options - Дополнительные опции
 * @param {boolean} options.skipInstances - Не заходить внутрь INSTANCE нод
 * @param {boolean} options.filterOrbPrefix - Не заходить внутрь компонентов без orb-* префикса
 */
function traverseNode(node, callback, skipSelf = false, options = {}) {
  // Если skipSelf === true, пропускаем сам узел, но проверяем детей
  if (!skipSelf) {
    callback(node);
  }
  
  // Если skipInstances включён и это INSTANCE - не заходим внутрь
  if (options.skipInstances && node.type === 'INSTANCE') {
    return;
  }
  
  // Если filterOrbPrefix включён - проверяем компоненты без orb-* префикса
  if (options.filterOrbPrefix && (node.type === 'INSTANCE' || node.type === 'COMPONENT')) {
    ...
    // Если нет orb- префикса - не заходим внутрь этого компонента
    if (!hasOrbPrefix) {
      return;
    }
  }
  ...
  if ('children' in node) {
    for (const child of node.children) {
      traverseNode(child, callback, false, options);
    }
  }
}
...
async function checkNumericVariables(options = {}) {
  try {
    ...
    console.log('📋 DSV: Настройки проверки:', {
      ...
      skipHiddenObjects: options.skipHiddenObjects !== false,
    ...
    for (const selectedNode of selection) {
      ...
      if (selectedNode.type === 'COMPONENT_SET') {
        ...
        }, true, traverseOptions); // skipSelf = true для COMPONENT_SET
        continue;
      }
      
      traverseNode(selectedNode, (node) => {
        nodesToCheck.push(node);
      }, false, traverseOptions);
    }
    
    console.log(`✓ Собрано ${nodesToCheck.length} нод для проверки`);
    
    if (options.skipInstances) {
      console.log(`ℹ️ DSV: Режим "Пропускать Instance" включён - дочерние элементы инстансов НЕ проверяются`);
    }
    
    if (options.filterOrbPrefix) {
      console.log(`ℹ️ DSV: Режим "Только orb-* компоненты" включён - компоненты без orb- префикса и их дети НЕ проверяются`);
    }
    ...
    let logMessage = `✓ Проверка завершена: ${nodesChecked} элементов, ${errors.length} ошибок`;
    if (skippedInstances > 0) {
      logMessage += `, пропущено ${skippedInstances} инстансов`;
    }
...
function isNodeOrParentHidden(node) {
  ...
  while (currentNode) {
    // ДИАГНОСТИКА: логируем каждую проверку
    const hasVisible = 'visible' in currentNode;
    const visibleValue = hasVisible ? currentNode.visible : 'N/A';
    
    if (depth === 0) {
      console.log(`🔍 DSV: Проверка видимости для "${getSafeNodeName(node)}" (${node.type})`);
    }
    console.log(`  ${depth === 0 ? '└─' : '  └─'} [${depth}] ${currentNode.type} "${getSafeNodeName(currentNode)}": visible=${visibleValue}`);
    
    // Проверяем свойство visible у текущей ноды
    if ('visible' in currentNode && currentNode.visible === false) {
      console.log(`  ✓ СКРЫТ на уровне ${depth}: ${currentNode.type} "${getSafeNodeName(currentNode)}"`);
      return true;
    }
  ...
  console.log(`  ✗ НЕ СКРЫТ (проверено уровней: ${depth + 1})`);
  return false;
}
...
async function checkNodeVariables(node, variables, mode, report, options = {}, savedTokens = null) {
  ...
  console.log(`🔧 DSV: options.skipHiddenObjects = ${options.skipHiddenObjects} (тип: ${typeof options.skipHiddenObjects})`);
  
  if (options.skipHiddenObjects !== false) {
    const isHidden = isNodeOrParentHidden(node);
    console.log(`🔧 DSV: Результат проверки isNodeOrParentHidden = ${isHidden}`);
    
    if (isHidden) {
      console.log(`✓ DSV: Пропущен скрытый объект: ${getSafeNodeName(node)} (${node.type})`);
      return;
    } else {
      console.log(`→ DSV: Объект НЕ скрыт, продолжаем проверку: ${getSafeNodeName(node)} (${node.type})`);
    }
  } else {
    console.log(`⊘ DSV: Настройка skipHiddenObjects ОТКЛЮЧЕНА, проверяем все объекты`);
  }
  
  // 3. Фильтрация по префиксу "orb-" для INSTANCE и COMPONENT
  if (options.filterOrbPrefix && (node.type === 'INSTANCE' || node.type === 'COMPONENT')) {
    const nodeName = getSafeNodeName(node);
    let hasOrbPrefix = nodeName.toLowerCase().startsWith('orb-');
  ...
  // 4. Проверяем настройку для INSTANCE нод
