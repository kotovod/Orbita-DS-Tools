console.log('=== ORBITA ICON CHECKER v3.0.4 START ===');
console.log('Timestamp:', new Date().toISOString());

// Основной код плагина Orbita Icon Checker
// VERSION: 3.0.4 (2025-10-16 23:30)

console.log('🔧 Orbita Icon Checker v3.0.4 загружен');
  ...
  for (const prop of propertiesToCheck) {
    ...
    if (prop.key === 'cornerRadius' && !boundVar && 'boundVariables' in node && node.boundVariables) {
      ...
      for (const corner of cornerProperties) {
        ...
        if (cornerValue !== undefined && cornerValue !== 0) {
          ...
          if (cornerBoundVar) {
            ...
            console.log(`  Value: ${cornerValue}`);
            console.log(`  ---`);
            
            // Поиск подходящего токена по значению
            const suggestedToken = findTokenByValue(variables, cornerValue, 'cornerRadius');
            
            console.log(`DSV: Поиск токена для угла ${corner.name}, значение: ${cornerValue}, найден:`, suggestedToken ? suggestedToken.name : 'нет');
      ...
      if (checkedIndividualCorners) {
        console.log(`DSV: ✓ Проверены раздельные углы для ${node.name}`);
        continue;
      }
    }
    
    // Логирование для отладки (можно удалить после тестирования)
    if (node.name && node.name.includes('DEBUG')) {
      console.log(`DSV: Проверка ${node.name} -> ${prop.key}:`, {
    ...
    if (boundVar) {
      ...
      if (Array.isArray(boundVar)) {
        ...
        if (boundVar.length > 0 && !hasValidBinding) {
          console.warn('DSV: boundVar массив есть, но нет валидных привязок для', prop.key, 'в', node.name);
        }
      ...
      if (Array.isArray(propertyValue)) {
        ...
        if (hasVisibleValues && shouldReportMissingVariable(node, prop.key)) {
          ...
          if ('boundVariables' in node) {
            ...
            console.log(`  Все boundVariables:`, Object.keys(node.boundVariables));
          }
  ...
  if (!variables.has(variableId)) {
    ...
    try {
      ...
      if (!variable) {
        ...
        console.warn(`DSV: Variable не найден в Figma API. Node: "${node.name}", Property: ${propertyName}, ID: ${variableId.substring(0, 20)}`);
        issues.push({
        ...
        if (tokenByName) {
          ...
          console.warn(`DSV: ❌ Токен не найден. Node: "${node.name}", Property: ${propertyName}, Token: "${variable.name}", isRemote: ${variable.remote}, Mode: ${mode}`);
...
async function bindTokenToProperty(nodeId, property, tokenId) {
  try {
    ...
    if (!variable && tokenName) {
      ...
      // Если не нашли в локальных - пробуем получить список всех используемых переменных из документа
      if (!variable) {
        console.log(`DSV: Не найден в локальных, ищем в используемых переменных документа...`);
        
        // Получаем все ноды на текущей странице
        const allNodes = figma.currentPage.findAll();
        const usedVariableIds = new Set();
        
        // Собираем ID всех используемых переменных
        ...
        console.log(`DSV: Найдено используемых переменных: ${usedVariableIds.size}`);
    ...
    if (!variable) {
      return {
        success: false,
        error: tokenName 
          ? `Токен "${tokenName}" не существует в Figma. Убедитесь что библиотека подключена.`
          : 'Токен не найден'
      };
    }
    ...
    console.log(`DSV: ✓ Токен "${variable.name}" привязан к свойству "${property}" элемента "${node.name}"`);
